...
def preprocess_image(path, temp_dir):
    img = clipImage(path)
...
#预处理 缩放图
def preprocessImage(pool, image_files, temp_dir):
    futures = []
    preproces_dir = create_specified_dir(temp_dir, "1_preprocess")

    for path in image_files:
        futures.append(pool.apply_async(preprocess_image, (path,temp_dir)))

    imageObjs = []
    for future in futures:
        img, path = future.get()
        imageObjs.append(img)
        new_file_name = get_specified_dir(preproces_dir, get_png_name_for_jpeg(path))
